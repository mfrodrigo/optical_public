% SOA_vsPower - SOA simulations versus input power
...
%   $Revision: 1.0. $Date: 23/5/2007

clear all
clc
disp('SOA versus input power simulation')
...
Pin_dBm = [-40.0:5:10];  % input power vector in dBm
...
%         disp(['error = ' num2str(tol) ' %'])  % display current error
    ...
    Nout(Ipin) = 2*eta_out*(1-R2)*sum(K.*Nsp(Nz+1,:).*E);  % output total ASE power
    ...
    sigmaN(Ipin) = interp1(E,sigmaN_spec,Es,'spline'); % interpolating to find output ASE power spectral density at signal wavelength   
    
end

Pout_dBm = db(Pout/1e-3,'power');
...
subplot(2,1,1)
[AX,H1,H2] = plotyy(Pin_dBm,db(Gain,'power'),Pin_dBm,db(NF,'power'),'plot');
xlabel('Input power (dBm)','Fontsize',14);
title(['Fibre-to-fibre gain and noise figure at ' num2str(wavelength_s/1e-9) ' nm. Bias current = ' num2str(bias/1e-3) ' mA'],'Fontsize',14)

set(get(AX(1),'Ylabel'),'String','Fibre-to-fibre gain (dB)','Fontsize',14)
set(AX(1),'XLim',[min(Pin_dBm) max(Pin_dBm)]); 
set(AX(1),'YLim',[-10 30],'Ytick',[-10:5:30]); 
set(H1,'LineStyle','-','Marker','+')

set(get(AX(2),'Ylabel'),'String','Noise figure (dB)','Fontsize',14)
set(H2,'LineStyle','-','Marker','o'); 
set(H2,'LineStyle','-.');
set(AX(2),'XLim',[min(Pin_dBm) max(Pin_dBm)]); 
set(AX(2),'YLim',[0 20],'Ytick',[0:5:20]);

subplot(2,1,2)
[AX,H1,H2] = plotyy(Pout_dBm,db(Gain,'power'),Pout_dBm,db(NF,'power'),'plot');
xlabel('Output power (dBm)','Fontsize',14);
title(['Fibre-to-fibre gain and noise figure at ' num2str(wavelength_s/1e-9) ' nm. Bias current = ' num2str(bias/1e-3) ' mA'],'Fontsize',14)
